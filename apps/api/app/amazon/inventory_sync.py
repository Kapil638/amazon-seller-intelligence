"""12B.6B — Inventory synchronization *trigger* (HTTP-facing wrapper).
...
enqueues (or reports the existing) durable `run_type='inventory'` job row
via `AmazonIngestionRunRepository`. A separate worker process
(`app.amazon.inventory_worker`) claims and processes queued jobs; nothing
...
Reuses `AmazonInventoryIngestionService._check_scope` for ownership/
eligibility validation rather than duplicating those checks — the same
helper the durable worker path (`process_claimed_job`) already uses.
Unlike Listings, this scope check returns no `selling_partner_id`:
`getInventorySummaries` carries no seller ID in its URL (query-string
only — see `inventory_client.py`'s module docstring), so there is
nothing Inventory-specific for this trigger to resolve beyond the
marketplace and connection.
"""
...
from app.amazon.inventory_ingestion import AmazonInventoryIngestionService, _ClaimFailure
...
class InventorySyncJobStatus(BaseModel):
    """Sanitized durable-job progress contract (12B.6B). Never carries an
...
class InventorySyncTriggerOutcome:
...
def _job_status_from_row(row: AmazonIngestionRun) -> InventorySyncJobStatus:
    # `retry_count` only increments on a genuine re-claim from
    # `waiting_to_retry` (see `claim_next_inventory_job`); a job that has
...
class AmazonInventorySyncTriggerService:
    """Enqueues a durable Inventory job, or reports the caller's existing
    one. Never resolves secrets and never calls Amazon itself —
    `AmazonInventoryIngestionService`/`app.amazon.inventory_worker` own all
    ...
    def trigger(self, marketplace_participation_id: UUID) -> InventorySyncTriggerOutcome:
        ...
        with session_scope() as session:
            ...
            if participation is None:
                return InventorySyncTriggerOutcome(reason="scope_not_found")
            seller_account_id = participation.seller_account_id

            try:
                _marketplace_id, connection_snapshot, region, environment = (
                    AmazonInventoryIngestionService._check_scope(
                        session,
                ...
                return InventorySyncTriggerOutcome(reason=exc.reason)

            runs = AmazonIngestionRunRepository(session)

            active = runs.get_active_inventory_run(organization_id, marketplace_participation_id)
            if active is not None:
                return InventorySyncTriggerOutcome(reason="already_running", job=_job_status_from_row(active))

            # Cooldown-relevant, not `get_latest_inventory_run` — a
            # `cancelled_before_start` administrative cancellation never
            # made an Amazon call and must never itself extend a cooldown
            # (see `get_latest_cooldown_relevant_inventory_run`'s
            ...
            latest = runs.get_latest_cooldown_relevant_inventory_run(organization_id, marketplace_participation_id)
            if latest is not None and cfg.inventory_sync_trigger_cooldown_seconds > 0:
                ...
                # here, `get_active_inventory_run` has already confirmed no
                ...
                # (`complete_inventory_run`, and the stale-lease
                # `timed_out` reclaim) unconditionally sets `completed_at`.
                # `created_at` is used
                ...
                retry_allowed_at = ensure_utc(anchor) + timedelta(seconds=cfg.inventory_sync_trigger_cooldown_seconds)
                if ensure_utc(db_now) < retry_allowed_at:
                    return InventorySyncTriggerOutcome(
            ...
            # `claim_next_inventory_job`'s own `started`-only counts.
            if (
                runs.count_queued_inventory_runs_for_organization(organization_id)
            ...
            if not claim.claimed:
                ...
                raced = runs.get_active_inventory_run(organization_id, marketplace_participation_id)
                return InventorySyncTriggerOutcome(
            ...
            return InventorySyncTriggerOutcome(reason="queued", job=_job_status_from_row(new_row))

    def get_status(self, marketplace_participation_id: UUID, run_id: UUID) -> InventorySyncJobStatus | None:
        ...
        requires the row to be a Inventory run for this exact
...
def get_amazon_inventory_sync_service() -> AmazonInventorySyncTriggerService:
    return AmazonInventorySyncTriggerService()
