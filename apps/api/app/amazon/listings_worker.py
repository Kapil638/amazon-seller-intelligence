...
# app/amazon/worker_heartbeat.py's module docstring.
WORKER_TYPE = "listings"
...
class ListingsWorker:
    ...
    def __init__(
        ...
        *,
        ...
        # workers and assert on exact asyncio.sleep call sequences (a
        # background heartbeat task sleeping independently would otherwise
        # interleave with — and break — those assertions). Always True for
        # any real worker process; see main() below, which never passes
        # this parameter.
    ...
    async def run_forever(self) -> None:
        logger.info("amazon listings worker started worker_type=%s instance_id=%s", WORKER_TYPE, self._instance_id)
        cfg = self._cfg()
        heartbeat = None
        if self._enable_heartbeat:
            heartbeat = start_heartbeat_loop(WORKER_TYPE, instance_id=self._instance_id, settings=cfg)
            logger.info(
                "amazon listings worker database connection OK; claim loop ready worker_type=%s", WORKER_TYPE
            )
        try:
        ...
        logger.info("amazon listings worker stopped worker_type=%s", WORKER_TYPE)
