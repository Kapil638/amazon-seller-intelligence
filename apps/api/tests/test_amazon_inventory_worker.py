"""12B.6B — `InventoryWorker` (app.amazon.inventory_worker). No live Amazon
call: the FBA Inventory client is fully faked, exactly like
`test_amazon_inventory_ingestion.py`. This file tests only the worker's
own polling/claim/loop/signal/gate responsibilities; ingestion/
reconciliation logic is already covered exhaustively there and is not
re-tested here. `claim_next_inventory_job`'s real PostgreSQL `SKIP
LOCKED`/advisory-lock concurrency would need its own guarded
disposable-Postgres test — out of scope for this pass given no live-
migration/live-Postgres action is authorized in this milestone (mirrors
`test_amazon_sales_traffic_worker.py`'s own identical scoping note).
"""
...
from app.amazon.inventory_models import Granularity, InventoryPage, InventoryPageProvenance, InventorySummary
from app.amazon.inventory_worker import (
...
def _provenance() -> InventoryPageProvenance:
    return InventoryPageProvenance(
        operation="getInventorySummaries", region="na", endpoint_host="sellingpartnerapi-na.amazon.com",
        fetched_at=datetime.now(UTC), http_status=200, api_model_version="fba-inventory-api-model/v1",
        attempt_count=1,
    )


def _page(seller_sku: str = "SKU-1") -> InventoryPage:
    return InventoryPage(
        granularity=Granularity(granularityType="Marketplace", granularityId=MARKETPLACE),
        summaries=[InventorySummary(sellerSku=seller_sku, condition="NewItem", totalQuantity=5)],
        next_token=None, marketplace_id=MARKETPLACE, page_token_used=None, provenance=_provenance(),
    )
...
def _test_settings(**overrides) -> Settings:
    fields = dict(
        ...
        inventory_sync_lease_duration_seconds=300,
        inventory_sync_max_global_concurrent_jobs=10,
        inventory_sync_max_concurrent_jobs_per_organization=10,
...
def _worker(
    *, pages: list | None = None, settings: Settings | None = None, resolver=None,
) -> tuple[InventoryWorker, _FakeInventoryClient]:
...
def _seed_scope() -> dict:
    ...
    with session_scope() as session:
        ...
        session.add(Organization(id=org_id, name="12B.6B Worker Test Org"))
        session.flush()
...
# --- run_once: successful claim + reconcile ---------------------------------


@pytest.mark.asyncio
async def test_run_once_claims_and_reconciles_a_queued_job() -> None:
    scope = _seed_scope()
    claim = _enqueue(scope)
    worker, client = _worker(pages=[_page()])

    claimed_something = await worker.run_once()

    assert claimed_something is True
    assert len(client.fetch_page_calls) == 1
...
async def test_run_once_processes_one_job_at_a_time_across_calls() -> None:
    ...
    worker, client = _worker(pages=[_page("SKU-A"), _page("SKU-B")])
    ...
    assert _get_run(claim_a.run_id).status == "succeeded"
    assert _get_run(claim_b.run_id).status == "succeeded"


# --- run_once: retryable failure reschedules --------------------------------


@pytest.mark.asyncio
async def test_run_once_reschedules_a_rate_limited_job_instead_of_failing_it() -> None:
    scope = _seed_scope()
    claim = _enqueue(scope)
    worker, client = _worker(pages=[SpApiRateLimitedError("slow down", retry_after_seconds=12.0)])
...
async def test_non_retryable_failure_terminalizes_immediately_without_retry() -> None:
    ...
    scope = _seed_scope()
    claim = _enqueue(scope)
    worker, client = _worker(pages=[SpApiInvalidRequestError("bad request")])
...
async def test_per_organization_limit_prevents_a_second_job_from_starting() -> None:
    ...
    claim_a = _enqueue(scope_a)
    with session_scope() as session:
        AmazonIngestionRunRepository(session).claim_next_inventory_job(
            lease_owner="existing-worker", lease_duration_seconds=300, max_global_active=10,
            max_active_per_organization=10,
        )
    claim_b = _enqueue(scope_b)

    settings = _test_settings(inventory_sync_max_concurrent_jobs_per_organization=1)
...
def test_main_refuses_to_start_when_not_explicitly_enabled(monkeypatch, value) -> None:
    monkeypatch.setenv("ASI_INVENTORY_WORKER_ENABLED", value)
...
def test_worker_enabled_env_var_is_independent_of_other_workers() -> None:
    import app.amazon.inventory_worker as inventory_worker_module
    ...
    env_vars = {
        ...
        inventory_worker_module._WORKER_ENABLED_ENV_VAR,
    }
    assert len(env_vars) == 4


# --- production-database guard: runtime-context declaration -----------------


def test_disabled_worker_never_declares_a_db_runtime_context(monkeypatch) -> None:
    monkeypatch.delenv("ASI_INVENTORY_WORKER_ENABLED", raising=False)
...
def test_enabled_worker_declares_the_inventory_worker_db_runtime_context(monkeypatch, value) -> None:
    monkeypatch.setenv("ASI_INVENTORY_WORKER_ENABLED", value)
    monkeypatch.delenv("ASI_DB_RUNTIME_CONTEXT", raising=False)
    ...
    monkeypatch.setattr("app.amazon.inventory_worker.get_settings", _raise_invalid)
    try:
        exit_code = main()
        assert exit_code == EXIT_CONFIGURATION_ERROR
        assert os.environ.get("ASI_DB_RUNTIME_CONTEXT") == "inventory_worker"
    finally:
