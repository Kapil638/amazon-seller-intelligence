...
def test_worker_a_loses_every_fenced_mutation_after_worker_b_reclaims(disposable_engine) -> None:
    ...
    with Session(disposable_engine) as session:
        ...
        assert repo.set_amazon_report(
            run.id, lease_owner="worker-a", amazon_report_id="r-pg-fencing-1", amazon_report_status="PENDING"
        )
        session.commit()

    # 2. A's lease expires.
    with Session(disposable_engine) as session:
        row = session.get(AmazonAdsReportRun, run.id)
    ...
    with Session(disposable_engine) as session:
        repo = AmazonAdsReportRunRepository(session)
        assert repo.heartbeat(run.id, lease_owner="worker-a", lease_duration_seconds=300) is False
        assert repo.set_amazon_report(
            run.id, lease_owner="worker-a", amazon_report_id="r-hijack-attempt", amazon_report_status="PENDING"
        ) is False
        assert repo.update_amazon_status(run.id, lease_owner="worker-a", amazon_report_status="COMPLETED") is False
        assert repo.mark_retry(
            run.id, lease_owner="worker-a", next_retry_at=datetime.now(UTC) + timedelta(seconds=30),
            failure_class="stale_worker_attempt", failure_detail="worker-a should never win this",
        ) is False
        assert repo.mark_failed(
            run.id, lease_owner="worker-a", failure_class="stale_worker_attempt", failure_detail="worker-a should never win this"
        ) is False
        assert repo.mark_succeeded(run.id, lease_owner="worker-a", records_ingested=999) is False
        session.commit()
    ...
    with Session(disposable_engine) as session:
        repo = AmazonAdsReportRunRepository(session)
        assert repo.mark_succeeded(run.id, lease_owner="worker-b", records_ingested=1) is True
        AmazonAdsSyncCheckpointRepository(session).advance(
            org_id, profile_id, synced_through_date=date(2026, 9, 1), report_run_id=run.id
        )
        session.commit()
    ...
    with Session(disposable_engine) as session:
        ...
        assert checkpoint.last_successful_report_run_id == run.id
...
def test_fact_persistence_and_completion_are_atomic_with_the_ownership_check(disposable_engine) -> None:
    ...
    with Session(disposable_engine) as session:
        ...
        ok = AmazonAdsReportRunRepository(session).mark_succeeded(run.id, lease_owner="worker-a", records_ingested=1)
        assert ok is False
