...
| `17_get_order_response_envelope.json` | **12B.4C addition.** The literal `GetOrderResponse` envelope (`{"order": {...}}`) — every prior fixture in this directory is `SearchOrdersResponse`-shaped (`{"orders": [...]}`) because 12B.4A only exercised `searchOrders`. Independently pins the `getOrder` operation's actual top-level response shape (no earlier fixture proved this — 12B.4C's initial `getOrder` tests only wrapped a `searchOrders` fixture's order object inline). Also the only fixture in this directory populating `Order.packages`/`OrderPackage` (`packageStatus`, `carrier`, `trackingNumber`, timestamps), so it doubles as the first fixture-driven coverage of that DTO. |
